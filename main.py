...
app = Flask(__name__)
app.secret_key = os.environ['flashMSG']
...
def edit(id):
  if request.method == "GET":
    ...
    if result == 1:
      flash(f"{name} with {teacher} was updated successfully", "success")
...
def add_dept():
  if request.method == "GET":
    ...
    try:
      ...
      flash(f"ERROR: {deptName} was NOT added.", "danger")
    if dept_id != 0:
      # flash a success message
      flash(f"{deptName} was added successfully","success")
    return redirect("/show")
